...
async fn then_task_runs_after(world: &mut CtKWorld, task_name: String, predecessor: String) {
    ...
    let predecessor_timestamp = get_task_timestamp(&world.workflow_events, &predecessor)
        .expect(&format!("Task '{}' was not found in workflow events", predecessor));

    assert!(
        task_timestamp > predecessor_timestamp,
        "Task '{}' (at {:?}) should run after '{}' (at {:?}), but it didn't",
        task_name, task_timestamp, predecessor, predecessor_timestamp
    );
}
