...
use jackdaw::providers::persistence::mem::InMemoryPersistence;
use serde_json::json;
...
async fn setup_test_engine() -> (Arc<DurableEngine>, Arc<InMemoryPersistence>) {
    let persistence = Arc::new(InMemoryPersistence::new());
    let cache = Arc::new(InMemoryCache::new()) as Arc<dyn CacheProvider>;
    let engine = Arc::new(
        DurableEngine::new(
...
fn load_workflow(fixture_path: &str) -> WorkflowDefinition {
    ...
    serde_yaml::from_str(&workflow_yaml)
        .unwrap_or_else(|e| panic!("Failed to parse workflow: {}\nError: {}", fixture_path, e))
}
...
async fn test_schedule_every_seconds() {
    let (engine, persistence) = setup_test_engine().await;
    ...
    let execution_count = count_workflow_executions(&persistence).await;

    assert_eq!(
        execution_count, 3,
        "Expected 3 workflow executions (at 0s, 2s, 4s), but found {}. \
         Scheduler not yet implemented - currently only runs once.",
        execution_count
    );
}
...
async fn test_schedule_no_schedule_runs_once() {
    let (engine, _persistence) = setup_test_engine().await;
...
///
/// Note: This is a placeholder for TDD. The real implementation would need
/// a way to list all instance IDs from persistence, which doesn't exist yet.
/// For now, this always returns 1 (representing single non-scheduled execution)
async fn count_workflow_executions(_persistence: &Arc<InMemoryPersistence>) -> usize {
    // TODO: Implement once scheduler is added and persistence supports listing all instances
    // Need something like: persistence.list_all_instance_ids()
    // Then for each instance_id, check if WorkflowStarted event exists

    // Placeholder: Until scheduler is implemented, workflow only runs once
    1
}
...
async fn get_execution_timestamps(
    _persistence: &Arc<InMemoryPersistence>,
